...
The loan dataset is composed 13 variables and 614 observations. Eight of the variables are factors and 5 are numeric. The dataset has a few missing values but is mostly complete and no variable has more than 10% missing data. Imputation of the missing values will be a step in the data processing. We also note that the Loan_ID variable is simply an index and holds no valuable information, making it safe for removal. The Credit_History variable is coded as numeric but it is a binary variable with two levels.
...
loan %>%  gather(key = "variable", value = "measurement", c('TotalIncome','LoanAmount','Loan_Amount_Term')) %>% 
  ...
  #scale_fill_manual(values = c("darkorange","purple","cyan4")) +
  theme_minimal() +
  ggtitle("Numeric Variable Boxplots")
```


```{r}
loan %>%  gather(key = "variable", value = "measurement", c('Gender':'Self_Employed')) %>% 
