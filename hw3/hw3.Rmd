...
The KNN algorithms requires minor data processing. Firstly, predictor values that are factors should be conversted to numeric. Secondly, because KNN uses distance between points to determine their classification, it is important for the points to be on the same scale. Here we use the `scale` argument to the `preProcess` parameter of the traning function to standardize each variable. The data is then split into training and testing sets 80%/20%. The test set contains 65 observations and the train set 268. 
...
We performed 10-fold crossvalidation to determine the optimal parameter k for our model. 
