...
To evaluate the simple tree model, we predict `Loan_Status` on the test set and calculate the performance metrics.
...
We prune tree based on CP value that has lowest cross validation error rate with atleaset 2 splits. The resulting tree is 3 levels levels and the most significant predictors are `Credit History` and `Total Income`
...
For classification datasets, the generally recommended subset of variables is the square root of the total number of predictor variables. In our dataset this is this is 3, since we have 10 total predictor variables
...
Next we want to see if we have generated enough trees within the forest so that the Out Of Bag (OOB Error) error rates are minimum. From the below we see that the  OOB error rate is decreasing with 1-20 trees, and rate stabilizes that at around 100 trees. Therefore for this dataset 500 trees is more than enough to minimize the OOB Error rate
...
`Credit History` and `Total Income` looks to be some of the most important predictors because:  
  
* They are usually at the root on most sampled trees  
* Have the lowest average depth  
...
Interestingly `Total Income`'s accuracy_decrease is mostly inline with the other predictors, much less than `Credit History`, but it has a higher gini decrease than the rest of the other predictors
