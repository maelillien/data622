...
```{r}
pca <- prcomp(select(mnist_raw_subset, -X1), center = TRUE, scale = FALSE)
summary(pca)
qqnorm(pca[["x"]][,1])
...
perf_dt <- data.frame(Model = "Base Decision Tree", Accuracy = dt_cm$overall[1], Kappa = dt_cm$byClass[1])
...
The final values used for the model were n.trees = 90, interaction.depth = 9, shrinkage = 0.1 and n.minobsinnode = 20.
...
```{r}
gbmPredict <- predict(gbm.fit, newdata = X_test) 
confusionMatrix(knnPredict, y_test)
```
