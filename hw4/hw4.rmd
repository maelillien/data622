...
The strategy that we will use for SVM is to compare several models specifically with and without Principal Component Analysis dimensionality reduction. For each model that is run  we hypertune the model parameters via a gridsearch of:
...
The models will perform a 10-fold Crossvalidation to prevent overfitting and arrive at the overall best parameters and accuracy rate below

## SVM(Dataset)
The dataset as is approach can be approached in two ways, to model against the entire dataset with questions (sums removed), and to model without the questions. 
...
Predicting gives us an accuracy there is fairly close to the training accuracy
```{r}
...
noquestions_method <- cbind(Reduction_Method = "Questions Removed", svm_m$best.parameters, Training_Accuracy = 1-svm_m$best.performance, Testing_Accuracy = sum(diag(svm_cm$table))/sum(svm_cm$table),
...
## SVM (PCA Reduction)
In the PCA Reduction approach we will aim to run SVM models by first reducing the dimensionality of the feature set. The reduced feature space will then be passed for prediction of the Suicide class via a SVM models. 
```{r}
adhd_raw2 <- adhd_raw %>% select(-MD.TOTAL, -ADHD.Total, -Initial, -Psych.meds.)
adhd_raw2 <- adhd_raw2[complete.cases(adhd_raw2), ]
adhd_raw2$Suicide <- as.factor(adhd_raw2$Suicide)
